...
Nash <- function(W, Hero = 0, Opp = 0, bestof = 5) {
  resultsdf <- as.data.frame(matrix(zolution(W, "Hero"), nrow = 1,ncol = 3, dimnames = list("", c("Primary", "Secondary", "Tertiary"))))
  resultsdf2 <- as.data.frame(matrix(zolution(W, "Opponent"), nrow = 1,ncol = 3, dimnames = list("", c("Primary", "Secondary", "Tertiary"))))
  ...
  winmatrix <- matrix(c(mw, fg, og), 1, 3, dimnames = list("", c("Match", "First game", "Subsequent games")))

  mw <- mijwr(Hero, Opp, bestof, W, "Opponent")
  fg <- unname(t(1-W)[1,1])
  og <- dagame(W, "Opponent")$solution[4]
  winmatrix2 <- matrix(c(mw, fg, og), 1, 3, dimnames = list("", c("Match", "First game", "Subsequent games")))
