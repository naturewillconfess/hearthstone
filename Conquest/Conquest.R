...
for (i in seq_along(comblist)) {
  won <- comblist[[i]]
  wonlen <- c(length(won[[1]]), length(won[[2]]))
  if (wonlen[1] == n) {
    V <- 1
  } else if (wonlen[2] == n) {
    V <- 0
  } else if (wonlen[1] == n-1) {
    V <- 1-prod(1-WM[!(1:n) %in% won[[1]], !(1:n) %in% won[[2]]])
  } else if (wonlen[2] == n-1) {
    V <- prod(WM[!(1:n) %in% won[[1]], !(1:n) %in% won[[2]]])
  } else {
    #Players options
    ...
    for (j in optP) {
      for (k in optO) {
        G[j,k] <- WM[j,k]*get(paste0("P", paste0(sort(c(won[[1]], j)), collapse = ""), "O", paste0(sort(won[[2]]), collapse = ""))) + (1-WM[j,k])*get(paste0("P", paste0(sort(won[[1]]), collapse = ""), "O", paste0(sort(c(won[[2]],k)), collapse = "")))
      }
    }
  ...
  assign(paste0("P", paste0(sort(won[[1]]), collapse = ""), "O", paste0(sort(won[[2]]), collapse = "")), value = V)
  print(i)
}
WM
comblist[[63]]
